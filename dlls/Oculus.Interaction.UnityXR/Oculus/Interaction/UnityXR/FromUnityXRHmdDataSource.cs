...
namespace Oculus.Interaction.UnityXR
{
	public class FromUnityXRControllerDataSource : DataSource<ControllerDataAsset>
	{
		...
		[SerializeField]
		private InputActionMap _leftHandControllerBindings = Enumerable.FirstOrDefault(InputActionMap.FromJson(ControllerActionMap));

		[SerializeField]
		private InputActionMap _rightHandControllerBindings = Enumerable.FirstOrDefault(InputActionMap.FromJson(ControllerActionMap.Replace("{LeftHand}", "{RightHand}")));
