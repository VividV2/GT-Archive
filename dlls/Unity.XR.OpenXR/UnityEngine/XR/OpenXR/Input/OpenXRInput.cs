...
namespace UnityEngine.XR.OpenXR.Input
{
	public static class OpenXRInput
	{
		...
		internal static void AttachActionSets()
		{
			...
			foreach (OpenXRInteractionFeature item in Enumerable.Where(Enumerable.OfType<OpenXRInteractionFeature>(OpenXRSettings.Instance.features), (OpenXRInteractionFeature f) => f.enabled && !f.IsAdditive))
			{
			...
			foreach (OpenXRInteractionFeature item2 in Enumerable.Where(Enumerable.OfType<OpenXRInteractionFeature>(OpenXRSettings.Instance.features), (OpenXRInteractionFeature f) => f.enabled && f.IsAdditive))
			{
		...
		private static bool CreateActions(List<OpenXRInteractionFeature.ActionMapConfig> actionMaps, Dictionary<string, List<SerializedBinding>> interactionProfiles)
		{
			foreach (OpenXRInteractionFeature.ActionMapConfig actionMap in actionMaps)
			{
				...
				List<string> list = Enumerable.ToList(Enumerable.Select(actionMap.deviceInfos, (OpenXRInteractionFeature.DeviceConfig d) => d.userPath));
				foreach (OpenXRInteractionFeature.ActionConfig action in actionMap.actions)
				{
					string[] array = Enumerable.ToArray(Enumerable.Union(Enumerable.ToList(Enumerable.Distinct(Enumerable.SelectMany(Enumerable.Where(action.bindings, (OpenXRInteractionFeature.ActionBinding b) => b.userPaths != null), (OpenXRInteractionFeature.ActionBinding b) => b.userPaths))), list));
