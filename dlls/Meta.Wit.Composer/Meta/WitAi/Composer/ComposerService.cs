...
public abstract class ComposerService : MonoBehaviour, ILogSource
{
	private class CurrentComposerRequest
	{
		...
		{
			SessionData.responseData = WitComposerResponseExtensions.GetComposerResponse(r);
		}
	}
	...
	{
		return Enumerable.FirstOrDefault(_requests, (CurrentComposerRequest compRequest) => compRequest?.Request != null && compRequest.Request.Equals(request)) != null;
	}
	...
	protected virtual void RouteVoiceServiceActivation(VoiceServiceRequest request)
	{
		...
		lock (_requestLock)
		{
			request.HoldTask = Task.WhenAll(Enumerable.Select(_requests, (CurrentComposerRequest check) => string.Equals(sessionData.sessionID, check?.SessionId) ? check?.Request?.Completion?.Task : null));
			_requests.Add(item);
		}
	...
	protected virtual void OnComposerResponse(ComposerSessionData sessionData, WitResponseNode response)
	{
		...
		{
			sessionData.responseData = WitComposerResponseExtensions.GetComposerResponse(response);
