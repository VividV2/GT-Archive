...
namespace Newtonsoft.Json
{
	public abstract class JsonWriter : IDisposable
	{
		...
		internal Task AutoCompleteAsync(JsonToken tokenBeingWritten, CancellationToken cancellationToken)
		{
			...
			{
				throw JsonWriterException.Create(this, StringUtils.FormatWith("Token {0} in state {1} would result in an invalid JSON object.", CultureInfo.InvariantCulture, tokenBeingWritten.ToString(), currentState.ToString()), null);
			}
			_currentState = state;
		...
		internal Task InternalWriteEndAsync(JsonContainerType type, CancellationToken cancellationToken)
		{
			...
			while (levelsToComplete-- > 0)
			{
				...
				if (_currentState == State.Property)
				{
					...
					if (!AsyncUtils.IsCompletedSuccessfully(task))
					{
		...
		{
			List<State[]> list = Enumerable.ToList(StateArrayTemplate);
