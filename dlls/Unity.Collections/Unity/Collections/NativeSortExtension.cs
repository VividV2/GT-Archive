...
namespace Unity.Collections
{
	...
	public static class NativeSortExtension
	{
		...
		{
			IntroSortStruct<T, DefaultComparer<T>>(NativeArrayUnsafeUtility.GetUnsafePtr(array), array.Length, default(DefaultComparer<T>));
		}
		...
		{
			T* unsafePtr = (T*)NativeArrayUnsafeUtility.GetUnsafePtr(array);
		...
		{
			IntroSort<T, U>(NativeListUnsafeUtility.GetUnsafePtr(list), list.Length, comp);
		}
		...
		{
			return SortJob(NativeListUnsafeUtility.GetUnsafePtr(list), list.Length, comp);
		}
		...
		{
			T* unsafePtr = (T*)NativeSliceUnsafeUtility.GetUnsafePtr(slice);
		...
		{
			return SortJob((T*)NativeSliceUnsafeUtility.GetUnsafePtr(slice), slice.Length, default(DefaultComparer<T>));
		}
		...
		{
			return SortJob((T*)NativeSliceUnsafeUtility.GetUnsafePtr(slice), slice.Length, comp);
		}
		...
		{
			return BinarySearch((T*)NativeSliceUnsafeUtility.GetUnsafeReadOnlyPtr(slice), slice.Length, value, comp);
		}
